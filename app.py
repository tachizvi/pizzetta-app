...
# --- 2. עובד: המשימות שלי (ממשק היברידי: הקלדה + כפתורים) ---
            ...
            for idx, row in pending.iterrows():
                with st.expander(f"ספירה עבור: {row['ספק']}", expanded=True):
                    ...
                    for _, p_row in s_prods.iterrows():
                        ...
                        # מבנה היברידי: [ -1 ] [ הקלדה ] [ +0.5 ] [ +1 ]
                        col_minus, col_input, col_p_half, col_p_one = st.columns([1, 2, 1, 1])
                        
                        with col_input:
                            # שדה הקלדה שמתעדכן מה-session_state וגם מעדכן אותו
                            val = st.number_input("כמות", min_value=0.0, step=0.5, key=f"input_{key}", 
                                                 value=st.session_state[key], label_visibility="collapsed")
                    ...
                    if st.button(f"✅ שלח ספירת {row['ספק']}", key=f"sub_{idx}"):
                        tasks_ws.update_cell(idx + 2, 2, "בוצע ✅")
                        tasks_ws.update_cell(idx + 2, 4, str(worker_counts))
                        for k in list(st.session_state.keys()):
                            if k.startswith("count_"): del st.session_state[k]
                        st.success("נשלח!")
                        st.rerun()
    ...
    if 'סטטוס' in df_tasks.columns:
        ...
        if ready.empty:
            ...
            for idx, t_row in ready.iterrows():
                ...
                if st.button(f"ייצר הודעה ({s})", key=f"msg_btn_{idx}"):
